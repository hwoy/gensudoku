...
unsigned int sgf_findvalueunique(struct sgs_game* game, unsigned int x, unsigned int y)
{
    ...
    for (i = 0; i < S_SQR; i++) {
        if (i != x)
            set |= sgf_getvalue_p(game, i, y);
    }

    for (i = 0; i < S_SQR; i++) {
        if (i != y)
            set |= sgf_getvalue_p(game, x, i);
    }

    for (m = i = (y / S_ZSQR) * S_ZSQR; i < m + S_ZSQR; i++) {
        for (n = j = (x / S_ZSQR) * S_ZSQR; j < n + S_ZSQR; j++) {
            if (j != x || i != y)
                set |= sgf_getvalue_p(game, j, i);
        }
    }

    m = sgf_getvalue_p(game, x, y);
    for (i = 0; i < S_SQR; i++) {
        if ((m & POW2A(i)) && !(set & POW2A(i)) && !sgf_getvalue(game, x, y))
            return i + 1;
    }
...
unsigned int sgf_getobstruct(struct sgs_game* game)
{
    ...
    for (k = 0, i = 0; i < S_SQR; i++) {
        for (j = 0; j < S_SQR; j++) {
            ...
            if (sgf_getvalue_p(game, j, i) == 0 && sgf_getvalue(game, j, i) == 0)
                k++;
        }
    }
    return k;
    ...
    for (z = 0; z <= S_ZSQR * 2; z += 3) {
        for (y = z; y < S_ZSQR + z; y++) {
            for (x = z; x < S_ZSQR + z; x++) {
                if ((x % S_ZSQR) >= (S_ZSQR) || (y % S_ZSQR) >= (S_ZSQR))
                    continue;

                if (sgf_getvalue(game, x, y) != 0)
                    continue;
